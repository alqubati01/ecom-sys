...
if (isset($_SESSION['admin_login']) && $_SESSION['admin_login'] === true) 
{
    ...
    $_POST['address'], $_POST['phone'], $_POST['editUser'])
    && !empty($_POST['id']) && !empty($_POST['first_name']) && !empty($_POST['last_name']) 
    && !empty($_POST['username']) && !empty($_POST['address']) && !empty($_POST['phone']) && !empty($_POST['editUser']))
    {
        if (preg_match('/^[a-z0-9 ]*$/i', $_POST['username'])) 
        {
            try 
            {
                ...
                if ($stmt->rowCount())
                {
                    ...
                    if ($stmt->rowCount()) 
                    {
                        echo json_encode(['status'=> 202, 'message'=> 'تم تعديل العميل بنجاح']);
                        $pdo->commit();
                        exit();
                    }
                }
